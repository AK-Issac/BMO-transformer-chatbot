...
class QLearningAgent:
    def __init__(self, action_space, state_size, learning_rate=0.1, discount_factor=0.99, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.01):
    ...
    def update(self, state, action, reward, next_state):
        # Assuming next_state is a tensor
        if next_state.numel() > 1:
            # If next_state has multiple values, use torch.any or other appropriate logic
            if torch.any(next_state >= self.q_table.shape[0]):
                # Handle the case where next_state is out of bounds
                print("next_state is out of bounds!")
                return

        # Convert to integer if single value
        if next_state.numel() == 1:
            next_state_index = int(next_state.item())
...
class ChatbotEnvironment:
    ...
    def calculate_reward(self, generated_text):
        ...
        """
        # Check if the generated text is coherent (e.g., not a random string)
        if self.is_incoherent(generated_text):
            return -1  # Penalize incoherent responses

        # Reward for appropriate length (not too short or too long)
        if len(generated_text) < 5:
            return -0.5  # Penalize for being too short
        elif len(generated_text) > 100:
            return -0.5  # Penalize for being too long

        # Reward if the text ends appropriately (not abruptly)
        if generated_text.strip().endswith(('.', '!', '?')):
            return 1  # Reward for good ending

        # Further reward can be added if context is better handled
        return 0.5  # Default small reward for normal text

    def is_incoherent(self, text):
        """
        Simple check to see if the generated text is incoherent. You can improve this
        with more sophisticated measures.
        """
        # Example incoherence check: length or absurd text patterns
        if len(text.strip()) == 0 or 'asdlkfjas' in text:
            return True
        return False
...
with open('rlagent-02.pkl', 'wb') as f:
    pickle.dump(model, f)
